...
### 修复
- 🐛 **用户锁内存泄漏修复** - 删除用户画像时清理 `_user_locks`，避免长期运行时内存积累
...
### 修复
- 🐛 **提示词格式保留修复** - 生成、优化、压缩、格式转换现在都会识别并保留当前提示词格式，不再把 Markdown/XML/JSON/YAML 一律按自然语言处理
- 🐛 **自动压缩流程修复** - 优化流程补齐自动压缩判断，待确认人格继续压缩时不再误回退到旧人格；同时在结果卡片中展示原始长度与是否发生自动压缩
- 🐛 **画像缓冲可靠性修复** - 画像缓冲区新增用户级锁，LLM 失败或返回空结果时会把批次消息恢复回缓冲区，避免静默丢消息
- 🐛 **画像监听范围修复** - 画像采集改为直接监听群聊/私聊消息，不再只覆盖触发 LLM 的消息
- 🐛 **LLM 配置健壮性修复** - Provider 配置缺失时自动回退到当前会话 Provider，整数配置解析也更稳健

### 改进
- ✨ **群聊画像体验增强** - 支持直接 `@用户` 管理画像；群聊默认仅监控当前群，同一用户可跨群追加 `group` 监控范围
- ✨ **群聊消息过滤增强** - 画像采集会保留 `@用户` 互动信息，同时自动忽略斜杠命令消息，减少画像噪音
- ✨ **交互文案统一** - “生成/优化/压缩”链路的状态反馈统一为“保存/选中/应用”三种语义，避免误以为保存后已自动应用到对话
- 📝 **文档与帮助同步** - README、插件元数据帮助和画像命令说明更新到当前行为
