...
- **智能意图识别** - `/快捷人格 智能` 命令智能理解你的意图

### 👤 用户画像
- **持续监控** - 实时监控指定用户的聊天消息，增量更新画像
- **画像提取** - 从对话中自动提取用户特征
- **多种模式** - 支持全局监控与按群监控；群聊里默认只监控当前群
- **画像卡片** - 美观的画像展示卡片
- **群聊友好** - 支持直接 `@用户` 管理监控，同一用户可跨多个群追加监控范围
...
| 命令 | 说明 |
|------|------|
| `/快捷人格 智能 <自然语言>` | 智能识别意图，自动执行对应操作 |
| `/qp 智能 <自然语言>` | 简写形式 |

**示例：**
```
/快捷人格 智能 生成一个傲娇猫娘        → 自动生成人格
/快捷人格 智能 让她更傲娇一点          → 自动优化人格
/快捷人格 智能 确认                    → 确认保存
/快捷人格 智能 切换到猫娘              → 激活人格
```
...
|------|------|------|
| `/画像 帮助` | 显示画像功能帮助 | - |
| `/画像 添加监控 <用户ID/@用户> [模式]` | 添加监控；群聊里默认 `group` | `/画像 添加监控 @小明` |
| `/画像 移除监控 <用户ID/@用户> [范围]` | 移除监控；群聊里默认只移除当前群 | `/画像 移除监控 @小明 current` |
| `/画像 监控列表` | 查看所有监控 | - |
...
2. 群聊里直接使用：/画像 添加监控 @目标用户
3. 等待目标用户在群里发送几条正常聊天消息
4. /画像 查看 @目标用户
```

### 群聊使用建议
```
1. 群聊里不写模式时，默认只监控当前群，适合大多数场景
2. 同一用户如果在多个群需要监控，分别去对应群执行一次 /画像 添加监控 @用户
3. /画像 移除监控 @用户 在群聊里默认只移除当前群；如需彻底移除可加 all
4. 斜杠命令消息会自动忽略，避免把管理指令混进画像
```
...
| `profile_min_messages` | 触发更新的最小消息数 | `10` |
| `profile_max_buffer_age` | 缓冲区最大保留时间(秒) | `300` |
| `profile_context_size` | 更新画像时读取的上下文条数 | `20` |
| `profile_include_bot` | 是否把机器人回复纳入上下文 | `true` |
