...
# 缺失字段分析模板（引导式生成用）
DEFAULT_MISSING_ANALYSIS_TEMPLATE = """分析以下人格描述，识别用户已提供的信息和缺失的关键字段。

用户描述: {description}

必须检查的字段（按重要性排序）:
1. speech_style - 说话风格/口癖（语气、常用词、句尾习惯等）【必填】
2. appearance - 外貌特征（发色、瞳色、服装风格等）【推荐】
3. background - 背景故事（职业、经历、身世等）【推荐】

可选字段（用户不填则跳过，不要求补充）:
4. user_identity - 用户身份（角色与用户的关系，如"QQ123456的主人"）【可选】
5. initial_attitude - 初始态度（对用户的态度）【可选，可根据性格推断】

分析规则：
- 如果用户描述中明确提到了某字段的相关信息，归入 provided
- 仅将【必填】和【推荐】字段中缺失的归入 missing
- 【可选】字段（user_identity, initial_attitude）即使缺失也**不要**归入 missing
- 用中文返回 label 和 hint

返回严格的 JSON 格式（不要有其他文字）:
{{
  "provided": ["字段标识1", "字段标识2"],
  "missing": [
    {{"field": "字段标识", "label": "显示名称", "hint": "简短提示，帮助用户理解要填什么"}}
  ]
}}

示例输出：
{{
  "provided": ["speech_style"],
  "missing": [
...
# 引导式生成模板（合并用户补充内容后生成）
DEFAULT_GUIDED_GEN_TEMPLATE = """# Role: 角色卡生成器
